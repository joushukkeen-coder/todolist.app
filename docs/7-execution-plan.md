...
| 2.4.0 | 2026-05-15 | joushukkeen | 다크 모드 추가: `users.dark_mode` 컬럼, `PATCH /users/me { darkMode }`, Header 토글 버튼(`useApplyDarkMode`/`useToggleDarkMode` 훅). 메모리 토큰 정책 유지하면서 사용자별 설정을 DB 영속화하여 재로그인 시에도 보존 |
| 2.5.0 | 2026-05-15 | joushukkeen | 다국어(ko/en/ja) 추가: `users.language` 컬럼 + CHECK 제약, `PATCH /users/me { language }`, Header 언어 셀렉터, `i18n/messages.ts` 경량 사전, `useApplyLanguage`/`useTranslation`/`useChangeLanguage` 훅 |
| 2.6.0 | 2026-05-15 | joushukkeen | 메인 화면을 월간 캘린더 뷰로 전환. `components/calendar/TodoCalendar.tsx` + `utils/calendar.ts` 추가. 셀당 dueDate 매칭 할일 제목 표시(최대 3개+N), 빈 셀 클릭→등록·할일 클릭→수정 동선. FE-09 카드 리스트는 캘린더 뷰로 대체됨 |
